...
//     // Use the correct bucket name from your Firebase
...
//       .then(() => console.log('✅ Bucket is accessible'))
//       .catch(err => console.warn('⚠️ Bucket test warning:', err.message));
      
//   } else {
//     console.log('⚠️ serviceAccountKey.json not found. Firebase features disabled.');
//   }
// } catch (error) {
...
//       console.log(`Found ${photos.length} active homepage photos with index`);
      
//     } catch (indexError) {
//       console.log('Index not ready, falling back to client-side filtering');
      
//       // Fallback: Get all and filter client-side
//       const allSnapshot = await db.collection('homepagePhotos').get();
...
//       console.log(`Found ${photos.length} active homepage photos (client-side filtered)`);
//     }
...
// app.get('/api/photos/by-year', authenticateUser, async (req, res) => {
//   try {
//     if (!firebaseInitialized) {
//       return res.status(503).json({ error: 'Firebase not initialized' });
//     }

//     console.log('Fetching photos grouped by year...');
...
//       const year = photo.uploadedAt.getFullYear();
...
//     console.log(`Found photos in years: ${Object.keys(photosByYear).join(', ')}`);
//     res.json(photosByYear);
...
//     let snapshot;
//     try {
//       snapshot = await db.collection('photos')
//         .orderBy('uploadedAt', 'desc')
//         .get();
//     } catch (err) {
//       return res.json([]);
...
// // ==================== HOMEPAGE PHOTO MANAGEMENT ROUTES ====================
...
// // Admin: Get all homepage photos (with fallback)
...
//     } catch (indexError) {
//       console.log('Index for admin query not ready, getting all documents');
...
//       const file = bucket.file(photoData.filePath);
//       await file.delete();
//       console.log('Deleted from Storage:', photoData.filePath);
//     } catch (storageError) {
//       console.warn('Storage file may not exist:', storageError);
//     }
...
// ║  🏠 Homepage Photos: http://localhost:${PORT}/api/homepage-photos ║
// ╚══════════════════════════════════════════════════════════╝
//   `);
// });
...
app.use(cors({
  // origin: ['http://localhost:3000', 'http://localhost:3001', 'http://127.0.0.1:3000', 'http://127.0.0.1:3001'],
    origin: 'https://rhea-hj1k.onrender.com',',
  credentials: true
}));
app.use(express.json());
app.use(express.urlencoded({ extended: true }));
