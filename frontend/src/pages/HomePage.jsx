  ...
  const fetchFolders = async () => {
    try {
      ...
      const foldersWithSamples = await Promise.all(
        response.data.map(async (folder) => {
          try {
            const photosResponse = await api.get(`/folders/${folder.id}?limit=8`);
            const allMedia = photosResponse.data.photos || [];
            const sampleMedia = allMedia.slice(0, 4);
            ...
            return {
              ...folder,
              stackItems: stackItems,
              samplePhotos: sampleMedia.map(m => m.url)
            };
          } catch (error) {
            return {
              ...folder,
      ...
      {!showFolderContent ? (
                ...
                folders.map(folder => (
                  ...
                  >
                    ...
                    <div className="folder-info-home">
                      ...
                      <span className="folder-photo-count-home">{folder.photoCount || 0} items</span>
                    </div>
                  </motion.div>
                ))
              )}
        ...
        <div className="gallery-page">
          <div className="gallery-content-overlay">
            ...
            <div className="folder-floating-header">
              {/* Circle Back Button - Left */}
              <button className="circle-back-btn" onClick={handleBackToFolders}>
                ←
              </button>

              {/* Center Section - Album Name + Badge Inline */}
              <div className="center-header-content">
                <h1 className="album-name-inline">{selectedFolder?.name}</h1>
                <span className="photo-count-badge-inline">
                  📸 {folderPhotos.length}
                </span>
              </div>
            </div>
                        ...
                        {isVideo ? (
                          <div className="polaroid-video-thumbnail" onClick={() => handleMediaClick(photo)}>
                            <video
                              ...
                              preload="metadata"
                              className="polaroid-video-preview"
                              poster={photo.thumbnailUrl || ''}  // ← Use thumbnail as poster
                              playsInline
                            />
                            <div className="polaroid-play-icon">▶</div>
                          </div>
