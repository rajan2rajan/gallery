                ...
                <div className="activity-list">
                  {photos.slice(0, 5).map(photo => (
                    <div key={photo.id} className="activity-item">
                      <img src={photo.url} alt={photo.title} className="activity-thumb" />
                      <div className="activity-info">
                        <span className="activity-title">{photo.title || 'Untitled'}</span>
                        <span className="activity-folder">📁 {photo.folderName || 'No folder'}</span>
                        <span className="activity-time">{formatDate(photo.uploadedAt)}</span>
                      </div>
                    </div>
                  ))}
                  {photos.length === 0 && (
                    <div className="no-activity">No recent uploads</div>
                  )}
                </div>
              </div>
            </div>
